    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional CHB outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes CHB and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::CHB_TO_CHB) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a CHB-to-CHB transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::CHB_ONLY;
        ...
        // If CHB-to-CHB fails, create a peg-out transaction
